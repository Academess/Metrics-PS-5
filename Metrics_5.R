...
for (i in 1:500){
  ...
  for (j in 11:31) {
    early_ur[, j] <- 5 + 2.5 * (j-10) + rnorm(15*1, mean=0, sd=1)
  }
  early_ur_vector <- c(t(early_ur))
  ...
  for (j in 26:31) {
    late_ur[, j] <- 5 + 2.5 * (j-25) + rnorm(15*1, mean=0, sd=1)
  }
  late_ur_vector <- c(t(late_ur))
  late_status_each <- c(rep(0, 25), rep(1, 6))
  late_status <- rep(late_status_each, time = 15)
...
# Plot the distribution of the coefficients
hist(beta, breaks = 20, main = "Distribution of Coefficients", xlab = "Values of Coefficients", ylab = "Density")
abline(v = mean(beta), col = "red", lwd = 2)